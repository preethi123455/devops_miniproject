...
resource "null_resource" "microshop_deployment" {
  ...
  provisioner "local-exec" {
    command = "docker build -t microshop-app:latest .."
  }

  # 2. Cleanup existing containers (to prevent name conflicts)
  provisioner "local-exec" {
    command = "docker rm -f microshop-prod microshop-dev || echo 'No existing containers to remove'"
  }

  # 3. Run Production Container (Port 80)
  provisioner "local-exec" {
    command = "docker run -d -p 80:80 --name microshop-prod microshop-app:latest"
  }

  # 4. Run Development Container (Port 8081)
  provisioner "local-exec" {
    command = "docker run -d -p 8081:80 --name microshop-dev microshop-app:latest"
  }
