...
- **`dev` Branch (Development)**: Commits to this branch simulate pushing to the development/testing environment. The Jenkins pipeline builds the Docker image and deploys it on **Port 8081**.
...
- **Deploy to DEV**:
  ...
  - Runs the newly built image on port `8081`.
...
1. **Push to `dev` Branch**:
   ...
   - Access the site at `http://<server-ip>:8081`.
